...
class ActorCritic(nn.Module):
    def __init__(self, num_inputs, num_outputs, std=0.0):
        ...
        self.critic = nn.Sequential(
            nn.Linear(num_inputs,    self.fc1_size),     nn.ReLU(),
            nn.Linear(self.fc1_size, self.fc2_size),     nn.ReLU(),
            nn.Linear(self.fc2_size, 1))

        self.actor = nn.Sequential(
            nn.Linear(num_inputs,    self.fc1_size),     nn.ReLU(),
            nn.Linear(self.fc1_size, self.fc2_size),     nn.ReLU(),
            nn.Linear(self.fc2_size, num_outputs))
...
class Agent():
    ...
    def learn(self, mini_batch_size, states, actions, log_probs, returns, advantages):
        for _ in range(self.epochs_per_train):
            for state, action, old_log_probs, return_, advantage in self.batch_iter(mini_batch_size, states, actions, log_probs, returns, advantages):
                ...
                surr2 = torch.clamp(ratio, 1.0 - self.max_policy_update, 1.0 + self.max_policy_update) * advantage
